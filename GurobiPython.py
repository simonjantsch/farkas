...
class GurobiModel:
    ...
    def __init__(self, c, A, b, lb=0, ub=GRB.INFINITY, modelSense='min',
        ...
        if sense is None:
            ...
            self.sense = sense
        self.varType = np.array([GRB.CONTINUOUS for _ in range(self.nVars)])
    ...
    def construct(self):
        ...
        x = model.addVars(range(self.nVars), lb=0, ub=GRB.INFINITY,
                          obj=self.obj, vtype=self.varType, name=self.varNames)

        lil = lil_matrix(self.A)
        for i, (row,data) in enumerate(zip(lil.rows, lil.data)):
            expr = LinExpr(0)
            found_some = False
            for j, coeff in zip(row,data):
                if coeff != 0:
                    expr.add(x[j]*coeff)
                    found_some = True
            if found_some:
                model.addConstr(expr,sense="<=",rhs=self.rhs[i])
        return model
